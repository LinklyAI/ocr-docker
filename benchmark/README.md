...
结果同时记录 Runpod `executionTime`、`delayTime`、客户端墙钟时间、吞吐、GPU 活跃成本/页，以及与 ground truth 的字符顺序相似度。GPU 活跃窗口完全由 Runpod 返回的 `delayTime + executionTime` 合并得到，不包含客户端轮询间隔；默认按 Runpod 当前 RTX 4090 Serverless Flex 公示价 `$1.10/h` 换算。优化版本至少应满足：成功率不下降、质量指标无显著回退，并在 P50/P95、吞吐或单位成本上产生可重复收益。
